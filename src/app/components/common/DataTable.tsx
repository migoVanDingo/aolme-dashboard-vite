...
const DataTable = ({ columns, data, actionLabel, actionKey, actionPath, rowPadding, border, borderRad }: any) => {
    ...
    <STable >
          ...
          <STableRow className={`${"head"} ${rowPadding && rowPadding} ${border && border} ${borderRad && borderRad}`} >
            {columns.map((col: any, index: number) => (
              <SRowCell key={index} className={"md"}>
                {col.label}
              </SRowCell>
            ))}
            <SRowCell className={"md"}>Actions</SRowCell>
          </STableRow>
          ...
          {data.map((row: any, rowIndex: number) => (
            <STableRow key={rowIndex} className={`${rowPadding && rowPadding} ${rowIndex % 2 === 0 ? "dark" : "light"} ${border && border} ${borderRad && borderRad}`}>
              {columns.map((col: any, colIndex: number) => (
                <SRowCell key={colIndex} className={`${"md"}`}>
                  {row[col.key]}
                </SRowCell>
              ))}
              <SRowCell className={"md"}>
                <SLink
                  onClick={() => navigate(`${actionPath}/${row[actionKey]}`)}
                >
                  {actionLabel}
                </SLink>
              </SRowCell>
